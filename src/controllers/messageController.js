...
exports.sendMessage = async (req, res, next) => {
  try {
    const { receiverId, content, category, priority, relatedTo, referenceId, metadata } = req.body;
    ...
    // If client provided metadata, merge it with our base metadata
    if (metadata) {
      // Check if replyTo was provided
      if (metadata.replyTo) {
        messageMetadata.replyTo = metadata.replyTo;
      }
    }
